...
def train_one_fold(fold, train_df, test_df, train_idx, val_idx, config=None):
    ...
    train_dataset, val_dataset, X_test_scaled, imputer, scaler = \
        preprocess_fold(train_df, test_df, train_idx, val_idx)
    ...
    scheduler = CosineAnnealingWarmRestarts(
        optimizer, T_0=cfg.get('T_0', T_0), T_mult=T_MULT, eta_min=ETA_MIN
    )
...
def train_all_folds(config=None):
    ...
    for fold, (train_idx, val_idx) in enumerate(fold_splits):
        mae, model_state, test_preds, history = \
            train_one_fold(fold, train_df, test_df, train_idx, val_idx, config)
